...
def homeView(request):
    ...
    if request.POST:
        ...
        if request.POST['action'] == "create_event":
            ...
            try:
                ...
                if form.is_valid():
                    firstCalendar = Calendar.objects.filter(Q(owner=request.user.pk) | Q(visible_for=request.user)).first()
                    form.set_calendar(firstCalendar.calendar_id)
                    form.save()
                    createEventForm = EventCreateForm()
                else:
                    createEventForm = form
    ...
    try:
        ...
        firstCalendar = Calendar.objects.filter(Q(owner=request.user.pk) | Q(visible_for=request.user)).first()
        if firstCalendar:
            context["events"] = getEventsForCalender(firstCalendar.calendar_id)
            context["selected_calendar"] = firstCalendar.calendar_id
